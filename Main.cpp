                     ...
                     int       nCmdShow)
{
	...
	inImage.setWidth(960);
	inImage.setHeight(540);
	inImage.setImagePath(argv[0].c_str());
	//inImage.setImagePath("usc.rgb");
	inImage.ReadImage();
	//inImage.SampleImg(4, 100);
	backImage.setWidth(960);
	backImage.setHeight(540);
	backImage.setImagePath(argv[1].c_str());
	backImage.ReadImage();
	vector<int> h;
	vector<int> s;
	vector<int> v;
	vector<int> dominant;
	inImage.countColor(h, s, v);
	inImage.findDominant(h, s, v, dominant, 0.95, 0.85);
	canvas = inImage;
	bool mask[960 * 540] = {};
	inImage.maskBackGround(dominant, mask);
	inImage.changeBackGround(mask, backImage);
	canvas = inImage;
...
// You are free to change the following code in any way in order to display the video
	...
	{	
		case WM_MOUSEMOVE:
		{
			...
			{
				vector<double> hsv;
				canvas.rgbToHsv((double)(unsigned char)canvas.getImageData()[3 * (p.y * canvas.getWidth() + p.x) + 2], (double)(unsigned char)canvas.getImageData()[3 * (p.y * canvas.getWidth() + p.x) + 1], (double)(unsigned char)canvas.getImageData()[3 * (p.y * canvas.getWidth() + p.x)], hsv);
				cout << "HSV:" << hsv[0] << " " << hsv[1] << " " << hsv[2] << endl;
			}
			//canvas = blackImage;
			//canvas.Magnify(p.x, p.y, 50, scale, blackImage, filterImage);
